...
/**
 * Renewal-failed email — sent when a Paystack card renewal fails
 * (invoice.payment_failed). Tells the client and gives them a link to pay.
 *
 * @param {object} opts
 * @param {string} opts.fullName
 * @param {string} opts.planName
 * @param {string} opts.planPrice    — e.g. "KES 15,000"
 * @param {string|null} opts.paymentUrl
 */
export function buildRenewalFailedEmail({ fullName, planName, planPrice, paymentUrl }) {
  const firstName = fullName.split(' ')[0];
  ...
  return layout(`
    ...
    <p style="margin:0 0 28px;color:#d3d3d3;font-size:15px;line-height:1.7;">
      We tried to process this month&rsquo;s payment for your <strong style="color:#f5f5f8;">${planName}</strong> membership (${planPrice}) but it didn&rsquo;t go through — usually a card limit or an expired card. Your training isn&rsquo;t interrupted yet; just complete the payment below to stay active.
    </p>

    ${paymentUrl ? `<table cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:32px;">
      <tr>
        <td align="center">
          <a href="${paymentUrl}"
             style="display:inline-block;background:#a60a08;color:#ffffff;font-size:13px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;text-decoration:none;padding:18px 40px;border-radius:8px;">
    ...
    </table>` : `<p style="margin:0 0 32px;color:#d3d3d3;font-size:15px;line-height:1.7;">
      Please reach out so we can sort out your payment.
    </p>`}

    <p style="margin:0;color:#555555;font-size:13px;line-height:1.6;">
      Already sorted it out? You can ignore this email. Questions? Reply here or reach us on Instagram
      <a href="https://instagram.com/amscperformance" style="color:#a60a08;text-decoration:none;">@amscperformance</a>.
    </p>
  `);
}
 ...
 */
export function buildPaymentReminderEmail({ fullName, planName, planPrice, paymentUrl, stage = '5d', daysOverdue = 0 }) {
  const firstName = fullName.split(' ')[0];

  // 'overdue' is only ever sent manually from the admin portal — the daily
  // cron fires 5d/1d/0d and then stops, so without this a late payer could
  // only be chased with wording that told them the payment was due today.
  // Tone is deliberately level: it assumes the payment was simply missed
  // rather than avoided, and makes one clear ask without repeating it.
  const dayCount = daysOverdue > 0
  ...
  const sublines = {
    ...
    overdue: dayCount
      ? `Your monthly AMSC payment is now ${dayCount} past due. If you've already sent it, thank you — please ignore this message. If not, you can settle it in a couple of minutes below.`
      : `Your monthly AMSC payment is still outstanding. If you've already sent it, thank you — please ignore this message. If not, you can settle it in a couple of minutes below.`,
  };
  const ctaLabels = {
    '5d': 'Pay Early &rarr;',
    '1d': 'Pay Now &rarr;',
    '0d': 'Pay Now &rarr;',
    overdue: 'Settle Payment &rarr;',
  };
  const eyebrows = {
  ...
  const eyebrow  = eyebrows[stage]  || 'Monthly Renewal';
...
/**
 * Camp registration confirmation — sent when a camp payment is confirmed AND
 * a slot was available (camp.confirm_registration() returned 'paid').
 * Carries the athlete's personal materials link.
 *
 * @param {object} opts
 * @param {string} opts.athleteName
 * @param {string} opts.guardianName
 * @param {string} opts.campName
 * @param {string} opts.campDatesDisplay  — e.g. "August 19–21, 2026"
 * @param {string} opts.venue
 * @param {string} opts.priceDisplay      — e.g. "KES 15,000"
 * @param {string} opts.materialsUrl
 */
export function buildCampConfirmationEmail({ athleteName, guardianName, campName, campDatesDisplay, venue, priceDisplay, materialsUrl }) {
  ...
  return layout(`
    ...
    <p style="margin:0 0 28px;color:#d3d3d3;font-size:15px;line-height:1.7;">
      <strong style="color:#f5f5f8;">${athleteName}</strong>&rsquo;s spot at ${campName} is confirmed. Payment received — see you there.
    </p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#1a1a1a;border:1px solid #222222;border-radius:8px;margin-bottom:28px;">
      <tr><td style="padding:20px 24px;">
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td style="padding:6px 0;color:#555555;font-size:12px;">Athlete</td>
            <td style="padding:6px 0;color:#f5f5f8;font-size:13px;font-weight:600;text-align:right;">${athleteName}</td>
          </tr>
          <tr>
            <td style="padding:6px 0;color:#555555;font-size:12px;">Dates</td>
            <td style="padding:6px 0;color:#f5f5f8;font-size:13px;text-align:right;">${campDatesDisplay}</td>
          </tr>
          <tr>
            <td style="padding:6px 0;color:#555555;font-size:12px;">Venue</td>
            <td style="padding:6px 0;color:#f5f5f8;font-size:13px;text-align:right;">${venue}</td>
          </tr>
          <tr>
            <td style="padding:6px 0;color:#555555;font-size:12px;">Amount Paid</td>
            <td style="padding:6px 0;color:#a60a08;font-size:15px;font-weight:800;text-align:right;">${priceDisplay}</td>
          </tr>
        </table>
      </td></tr>
    </table>
    ...
    <p style="margin:0 0 12px;color:#555555;font-size:13px;line-height:1.6;">
      This link is personal to ${athleteName} — bookmark it. Your Day 1 strength routine, recovery guide, nutrition sheet and shopping list are ready now; the individual Combine report card unlocks after Day 1 testing.
    </p>
    <p style="margin:0;color:#555555;font-size:13px;line-height:1.6;">
      Questions? Reply to this email or find us on Instagram
      <a href="https://instagram.com/amscperformance" style="color:#a60a08;text-decoration:none;">@amscperformance</a>.
...
/**
 * Camp waitlist notice — sent when a camp payment is confirmed but the camp
 * had already reached capacity (camp.confirm_registration() returned
 * 'waitlist'). Payment was captured, NOT refunded automatically — reassures
 * the guardian their money is accounted for while admin follows up.
 *
 * @param {object} opts
 * @param {string} opts.athleteName
 * @param {string} opts.guardianName
 * @param {string} opts.campName
 */
export function buildCampWaitlistEmail({ athleteName, guardianName, campName }) {
  const firstName = guardianName.split(' ')[0];
  ...
  return layout(`
    ...
    <p style="margin:0 0 20px;color:#d3d3d3;font-size:15px;line-height:1.7;">
      Your payment for <strong style="color:#f5f5f8;">${athleteName}</strong> went through, but this camp reached its
      20-athlete capacity at the same time — so ${athleteName} is now first on the waitlist rather than confirmed.
    </p>
    <p style="margin:0 0 28px;color:#d3d3d3;font-size:15px;line-height:1.7;">
      Your payment has been received and is accounted for. We&rsquo;ll be in touch directly within 48 hours to either
      confirm a spot if one opens up, or arrange a full refund.
    </p>

    <p style="margin:0;color:#555555;font-size:13px;line-height:1.6;">
      Questions in the meantime? Reply to this email or find us on Instagram
      <a href="https://instagram.com/amscperformance" style="color:#a60a08;text-decoration:none;">@amscperformance</a>.
    </p>
  `);
}
