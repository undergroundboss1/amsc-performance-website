...
export function validateCampRegistration(data, camp) {
  ...
  // ── Consent — both required, checked server-side, not just in the UI ────
  if (data.consentParticipation !== true) {
    errors.push('Please confirm permission for the athlete to participate, including the Ngong Hills activity.');
  }
  if (data.consentMedia !== true) {
    errors.push('Please confirm permission for photo/video use in AMSC and PBA marketing content.');
  }
